...
class GPEIChooser:
    ...
    def __init__(self, expt_dir, covar="Matern52", mcmc_iters=40,
                 pending_samples=100, noiseless=True):
    ...
    def next(self, grid, values, durations, candidates, pending, complete):
        ...
        if complete.shape[0] <= config.INITIAL_START:
    ...
    def compute_ei(self, comp, pend, cand, vals):
        if pend.shape[0] == 0:
            ...
            # TODO: Get the mean and variance of predictions from here @omid
            # print " >>>> ", 'comp', comp, 'pend', pend, 'cand', cand, 'vals', vals
            # print " >>>> ", vals
