...
def terasort(vms, env):
    ...
    argos_start(vms)
    hadoop.execute('/usr/bin/time -f \'%e\' -o terasort.out sudo -u hdfs hadoop jar /usr/lib/hadoop-0.20-mapreduce/hadoop-examples-2.6.0-mr1-cdh5*.jar terasort -D mapred.reduce.tasks={0} /terasort-input /terasort-output'.format(env.param('terasort:reducers')))
    argos_finish(vms)
